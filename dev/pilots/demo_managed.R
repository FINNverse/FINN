...
m <- finn(N_species = 1L,
  competition_process  = createProcess(~0, FINN::competition,  initSpecies = matrix(c(0.3, 0.2), 1, 2)),
  growth_process       = createProcess(~1, FINN::growth,       initSpecies = matrix(c(0.05, 0.045), 1, 2), initEnv = list(matrix(-1.7, 1, 1))),
  regeneration_process = createProcess(~1, FINN::regeneration, initSpecies = matrix(0.05, 1, 1),           initEnv = list(matrix(2.2, 1, 1))),
  mortality_process    = createProcess(~1, FINN::mortality,    initSpecies = matrix(c(0,0,0), 1, 3),        initEnv = list(matrix(-2.5, 1, 1))))
...
sched <- data.frame(year = seq(20, 100, by = 10))
sched$prescription <- I(replicate(nrow(sched),
  management_params(thinning_intensity = 0.2,
                    thinning_size_bias = graduated_bias(from = -0.6, to = 0.8, ref = 45),
                    target_diameter = 45, target_harvest_fraction = 0.5), simplify = FALSE))
...
df <- rbind(data.frame(year = b_ba$year, ba = b_ba$ba, regime = "unmanaged"),
            data.frame(year = m_ba$year, ba = m_ba$ba, regime = "managed (graduated thinning + Zielstaerke 45)"))
p <- ggplot(df, aes(year, ba, colour = regime)) +
  geom_vline(xintercept = seq(20,100,10), colour = "grey85", linewidth = 0.3) +
  geom_line(linewidth = 0.9) +
  scale_colour_manual(values = c("unmanaged" = "#1b9e77", "managed (graduated thinning + Zielstaerke 45)" = "#d95f02"), name = NULL) +
