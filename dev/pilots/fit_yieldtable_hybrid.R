...
m <- finn(
  ...
  growth_process       = createHybrid(~ ekl, hidden = c(20L, 20L), transformer = FALSE),  # DNN replaces growth
  ...
  regeneration_process = createProcess(~1, FINN::regeneration, initEnv = list(matrix(-12, 1, 1)),
                                       optimizeSpecies = TRUE, optimizeEnv = TRUE)  # ~0 recruits
)
...
p <- ggplot(pl, aes(age, value, colour = factor(Ekl))) +
  ...
  labs(title = "FINN (DNN-hybrid growth) fitted to the NW-FVA Fichte yield table",
...
ggsave("dev/pilots/fit_yieldtable_hybrid.png", p, width = 8, height = 5, dpi = 130)
cat("wrote dev/pilots/fit_yieldtable_hybrid.png\n")
