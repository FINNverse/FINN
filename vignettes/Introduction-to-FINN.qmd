...
Building and running the model needs a torch backend, so the simulation
below is shown but not executed when the page is knit; its result is
precomputed (`dev/precompute_vignettes.R`) and loaded from a cache.

```{r simulate1, eval = FALSE}
predictions <- list()
...
```{r intro-cache}
#| include: false
.vig <- readRDS(system.file("extdata", "vig_intro.rds", package = "FINN"))
# rebuild the `predictions` shape the plot below expects
predictions <- lapply(.vig, function(d) list(long = list(site = d)))
```
