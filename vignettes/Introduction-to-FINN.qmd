...
The structure of this model is similar to an expression like `~ 1 + \text{env}_1 + \text{env}_2`, where each environmental predictor has its own species-specific effect, and the neural network adjusts the coefficients $\beta_{i,k}$ for each species based on training data.
